...
class BuyerController {
    ...
    try {
      ...
      if (existingInspection) {
        throw new RouteError(
          ...
          "You have already requested an inspection for this property"
        );
      }
      ...
      const transaction = await DB.Models.Transaction.create({
        ...
        transactionReference:
          inspectionRequestData.transaction.transactionReference,
        transactionReceipt:
          inspectionRequestData.transaction.transactionReceipt,
      ...
      const mailTemplate = InspectionRequestWithNegotiation(
        inspectionRequestData.requestedBy.fullName,
        {
      ...
      const sellerTemplate = InspectionRequestWithNegotiationSellerTemplate(
        (property.owner as any).fullName || (property.owner as any).firstName,
        {
          ...
          }/seller-negotiation-inspection/${inspection._id.toString()}`,
        }
      );
  ...
  public async getInspection(inspectionId: string, userId?: string) {
    const inspection = await DB.Models.InspectionBooking.findById(inspectionId)
      .populate(
        "propertyId",
        "title location price propertyType briefType _id owner"
      )
      .populate("requestedBy", "fullName email phoneNumber")
      .populate(
        "transaction",
        "bank accountNumber accountName transactionReference transactionReceipt"
      );

...
export const listLawyerVerificationJobs = async (
  ...
  try {
    ...
    const visibleCatalog = status
      ? catalogJobs.filter((job) => job.status === status)
      : catalogJobs;

    return res.status(HttpStatusCodes.OK).json({
      success: true,
      data: [...jobs.map(redactBuyerUntilPaid), ...visibleCatalog],
    });
  } catch (err) {
    next(err);
  }
...
export const respondLawyerVerificationJob = async (
  ...
  try {
    ...
    const assigned = await DB.Models.DocumentVerification.findOne({
      _id: req.params.id,
      lawyerId: user._id,
    }).select("_id");
    if (!assigned) {

...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { email, firstName, lastName, phoneNumber, userType } =
        validator.validate(req.body, "userSignupSchema");

      const response = await userControl.signup(
        email,
        req.body.password,
        firstName,
        lastName,
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const { user, token } = await userControl.login({
        email: reqBody.email,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!deleted) {
        return res
          .status(404)
          .json({ error: "Brief not found or already deleted." });
      }

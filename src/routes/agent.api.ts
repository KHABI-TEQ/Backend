...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const response = await agentControl.confirmPropertyAvailability(
        requestId,
        isAvailable,
      );
      return res.status(HttpStatusCodes.OK).json({
 ...
 */
router.get(
  "/my-location-preferences",
  AuthorizeAction,
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const user = req.user as IUserDoc;
      const query = req.query;
      const preferences = await agentController.getMatchingPreferences(
        user,
        query,
      );
      return res.status(200).json({
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const property = await agentController.createBriefProperty(
        user,
        req.body,
        files,
        preferenceId,
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      //     })
      //   );
      const requests = await DB.Models.InspectionBooking.find({
        owner: user._id,
      }).populate([
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const response = await agentControl.acctUpgrade(agent, {
        companyAgent,
        meansOfId,
      });
      return res.status(200).json({
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { password, isAccountInRecovery, isDeleted, isInActive, ...data } =
        agent.toObject();

...
export const agentKycSchema = Joi.object({
  ...
  meansOfId: Joi.array().items(
    Joi.object({
      ...
      docImg: Joi.array().items(Joi.string().uri()).required().messages({
        "array.base": "Document images must be an array of URLs.",
        "any.required": "At least one document image is required.",
      }),
    })
  ...
  featuredListings: Joi.array()
    .items(
        ...
        .custom(async (value, helpers) => {
            ...
            if (!exists) {
            // This correctly signals a Joi validation failure
